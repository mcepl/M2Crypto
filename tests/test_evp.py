...
class EVPTestCase(unittest.TestCase):
    ...
    def test_hmac(self):
        ...
        if not fips_mode:  # Disabled algorithms
            self.assertEqual(
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='md5')
                ),
        ...
        if not fips_mode and m2.OPENSSL_VERSION_NUMBER < 0x30000000:
            self.assertEqual(
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='ripemd160')
                ),
        ...
        if m2.OPENSSL_VERSION_NUMBER >= 0x90800F:
            self.assertEqual(
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='sha224')
                ),
            ...
            self.assertEqual(
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='sha256')
                ),
                36273358097036101702192658888336808701031275731906771612800928188662823394256,
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='sha256')
            ...
            self.assertEqual(
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='sha384')
                ),
            ...
            self.assertEqual(
                util.octx_to_num(
                    EVP.hmac(b'key', b'data', algo='sha512')
                ),
    ...
    def test_get_ec(self):
        ...
        self.assertEqual(
            ec.compute_dh_key(ec), ec2.compute_dh_key(ec2)
        )
    ...
    def test_digest_verify(self):
        ...
        pkey.digest_verify_init()
        self.assertEqual(
            pkey.digest_verify(sig, b'test  message  not'), 0
        )
    ...
    def test_pkey_verify_crash(self):
        ...
        self.assertTrue(
            verify(res)
        )  # segmentation fault in *verify_final*
...
class CipherTestCase(unittest.TestCase):
    ...
    def try_algo(self, algo):
        ...
        self.assertEqual(
            otxt, ptxt, '%s algorithm cipher test failed' % algo
        )
    ...
    def test_AES(self):  # noqa
        ...
        for test in test_data:
            with self.subTest(
                msg="test_AES_{}".format(test_data.index(test))
            ):
                # Test with padding
                # encrypt
                k = EVP.Cipher(
                ...
                cbuf = io.BytesIO()
                ciphertext = hexlify(
                    self.cipher_filter(k, pbuf, cbuf)
                )
                ...
                pbuf = io.BytesIO()
                cbuf = io.BytesIO(
                    unhexlify(test['CT'] + cipherpadding)
                )
    ...
    def test_AES_ctr(self):  # noqa
        ...
        plaintext_value = (
            b'This is three blocks of text with unicode char \x03'
        )
        ...
        for key_size in [128, 192, 256]:
            ...
            cipher = EVP.Cipher(
                alg=alg, key=key_truncated, iv=nonce, op=op
            )
            ciphertext = cipher.update(plaintext_value)
            ciphertext = ciphertext + cipher.final()
            self.assertEqual(
                ciphertext, ciphertext_values[str(key_size)]
            )
            ...
            plaintext = cipher.update(
                ciphertext_values[str(key_size)]
            )
...
class PBKDF2TestCase(unittest.TestCase):
    def test_rfc3211_test_vectors(self):
        ...
        ret = EVP.pbkdf2(password, salt, iter, keylen)
        self.assertEqual(
            ret, unhexlify(b'6a8970bf68c92caea84a8df285108586')
        )
...
def suite():
    ...
    suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(EVPTestCase)
    )
    suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(CipherTestCase)
    )
    suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(PBKDF2TestCase)
    )
    suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(HMACTestCase)
    )
    return suite
